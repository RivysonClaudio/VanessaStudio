...
export class ServiceFormComponent {
  ...
  onEdit(){
    ...
    xhr.onload = () => {
      if (xhr.status == 201){
        this.admin.notify("As alterações foram salvas!", 3500);
        this.router.navigate(['/admin/servicos'])
      }else{
        this.admin.notify("Ação não realizada", 3500);
      }
    }
  ...
  onDelete(){
    this.admin.openModal(
      ...
      () => {
        ...
        xhr.open("DELETE", `http://localhost:8080/api/servicos/v1/${this.serviceId}`, false);

        xhr.setRequestHeader('Content-Type', 'application/json');
    
        xhr.onload = () => {
          if (xhr.status == 204){
            this.admin.notify("Serviço excluido com sucesso!", 3500);
            this.router.navigate(['/admin/servicos'])
          }else{
            this.admin.notify("Ação não realizada", 3500);
          }
